...
def hod_approve_request(request, request_id):
    ...
    # Generate QR code
    qr_data = f"""
    Student: {student_request.student.name}
    Tutor: {student_request.tutor.name if student_request.tutor else 'N/A'}
    HOD: {student_request.hod.name}
    Department: {student_request.department.name}
    Course: {student_request.course.name}
